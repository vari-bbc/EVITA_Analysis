...
pacman::p_load(readxl,ggplot2,table1,dplyr,ggprism,MASS,emmeans,ggbeeswarm,ggsci,
               ...
               forestmodel,tidycmprsk,cmprsk,ggsurvfit,ggfortify,ggh4x,corrplot)
...
vafs.before.ID = VAFS$Tumor_Sample_Barcode
...
#This is the visit variable
vdates$...2 = as.numeric(vdates$...2)

#Drop all rows with missing visit date
vdates = vdates[! is.na(vdates$...2),]

#File was provided without IDs being carried through, so we can use last value carried forward to fill them in
vdates$ID = zoo::na.locf(vdates$EVI2)

#Store IDs before trimming for proper accounting
vdates.before.ID = vdates$ID

#Trim hyphens
vdates$ID = sapply(strsplit(vdates$ID,"-"), function(x){ifelse(length(x) >1, paste0(x[[1]],as.numeric(x[[2]])),x)})

knitr::kable(data.frame(vdates.before.ID,vdates$ID),caption = "ID before and after vdate file",col.names = c("Before","After"))

#Make a visit column
vdates$visit = vdates$...2

#Merge VAFS information with years using ID and visit
...
#Split into 2 then merge (baseline and followup timepoint)
...
```{r}
#Patients without a follow-up visit
...
#exclude these patients from the clonal growth data
vafs = vafs[! vafs$ID %in% IDs.ex,]

```

```{r}
# Two patients have all their follow-up time points, but no VAFs, these should still be included as they were no issues they just didn't have any VAFs, adding them here. Patients 14 and 96
#This creates a data.frame from the main file where the main IDs are not in vafs
non.ev = df[df$ID %in% df$ID[which(! df$ID %in% vafs$ID)],]
#Exclude the IDs previously removed
non.ev = non.ev[! non.ev$ID %in% IDs.ex,]
knitr::kable(table(non.ev$ID),caption = "Stable with no VAFS")
#These 2 are stable
non.ev$Stability = "Stable"

#Add 14 and 96 into the vafs file for analysis
vafs = plyr::rbind.fill(vafs,subset(non.ev,select = c(ID)))

# Patients in the data.frame without a VAF at an included time point are imputed with 0.01, a sufficiently small value
for( i in non.ev$ID){
  #Grab their first visit, should be 1
  vafs[vafs$ID ==i,]$visit_1 = vdates[vdates$ID == i ,]$...2[1]
  #Grab their last visit, we know it's 5 for both because they had all 5 timepoints; verified manually and logically here
  vafs[vafs$ID ==i,]$visit_FU = tail(vdates[vdates$ID == i ,]$...2,1)
  vafs[vafs$ID ==i,]$AF_1 = 0.01
  vafs[vafs$ID ==i,]$AF_FU = 0.01
}

#No one has both baseline and FU missing which is expected as we removed everyone without a FU time
...
for(i in unique(vafs[is.na(vafs$Time_FU),]$ID)){
  #Create a temporary data.frame of only the current ID
  tmp =  vdates[vdates$ID == i ,]
  #Grab the time from their final (max) visit
  time = tmp$...4[tmp$visit == max(tmp$visit,na.rm=T)]
  #Change the vafs data frame to have the final time instead of NA
  vafs[is.na(vafs$Time_FU) & vafs$ID == i,]$Time_FU = time
}

#Create a dummy variable for 'new mutation'
vafs$Emergent = rep(0)
#Any VAF that was missing at AF_1 must have appeared later, these become 1s
vafs$Emergent[is.na(vafs$AF_1) ] = 1
#If missing AF at baseline give a small value so we can calculate a approximate growth rate
vafs$AF_1[is.na(vafs$AF_1)] = "0.01"

#Loop through each ID with a missing baseline time, make this time the date of the first time point collected. Similar logic to above loop
...
fit.init = lm(GR~Treatment_arm,vafs.c)

#Check Cook's distance, cutoff based on 4/(N-k-1)
cutoff = 4/(nrow(vafs.c)-1-1)
p.cd = ggplot(data=NULL,aes(x = 1:nrow(vafs.c),y=cooks.distance(fit.init)))+
  geom_point() +theme_prism() +
  geom_hline(yintercept = cutoff,color="red",linetype="dotted")+
  labs(caption="A few highly influential IDs")+
...
# Box plot of untrimmed points
clonal_trajectory_figure.untrimmed = ggplot(vafs.c,aes(x = Treatment_arm,y = GR,color=Treatment_arm)) + 
  geom_quasirandom() + 
  ...
  ylab(expression(bold("Cumulative"~log(frac("AF at follow-up","AF at baseline"))*"/year"))) +xlab("Treatment") +
  geom_point(data=agg.mu,aes(x = Treatment_arm,y = GR,color=Treatment_arm),size=4,shape=2)+
  theme(legend.position = "none") +
  annotate(geom="line",x= c(1,2),y =c(15,15) ) +
  annotate(geom="text",x = 1.5, y = 16, label = paste0("p = ", p),fontface="italic")+
  labs(caption = " ")

plot_grid(p.cd,clonal_trajectory_figure.untrimmed,labels=c("a","b"),
          label_size = 14,scale=0.9,rel_widths=c(1,0.33))
...
```{r}
# Box plot or raw cumulative GRs adding means as triangles and p-value with 2 sig-figs
...
FC = data.frame(confint(emmeans(fit,pairwise~Treatment_arm)$contrast))
FC$FC = exp(FC$estimate)
FC$asymp.LCL = exp(FC$asymp.LCL)
FC$asymp.UCL = exp(FC$asymp.UCL)
knitr::kable(subset(FC,select=c(FC,asymp.LCL,asymp.UCL)),caption="FC from rlm")
...
#Create an temp file for accounting purposes
...
knitr::kable(table(vafs$Hugo_Symbol)[order(table(vafs$Hugo_Symbol))],caption="Counts of each mutation")


# Function to plot GR of specific mutations with p-values
plot.spec.mut = function(muts) {
  vafs.mut = merge(vafs[vafs$Hugo_Symbol %in% muts,],subset(df,select=c(ID,Treatment_arm)))
  
  indv.genes = NULL
  for(i in muts){
    agg.vafs.mut = aggregate(GR~ID+Treatment_arm, vafs.mut[vafs.mut$Hugo_Symbol == i,],function(x) sum(x,na.rm=T))
  ...
  for(i in muts){
    ...
    print(ggplot(agg.vafs.mut,aes(x = Treatment_arm,y = GR,color=Treatment_arm)) + geom_quasirandom() + 
      geom_boxplot(fill=NA,outliers = F) + theme_prism() + scale_color_manual(values=c("black",pal_nejm()(4)[c(3)])) + ggtitle(i)+xlab("Treatment") + annotate(geom="line",x= c(1,2),y =c(y*1.05,y*1.05) ) + annotate(geom="text",x = 1.5, y = y*1.13, label = paste0("FDR p = ", FDR),fontface="italic")+ylab("Growth rate")+theme(legend.position = "none")+xlab("Treatment"))
  }
}

plot.spec.mut(c("TET2","DNMT3A","SRSF2","ASXL1","ZRSR2" ))
```
...
knitr::kable(summary(emmeans(fit,pairwise~Treatment_arm)),caption="emmeans and contrast")
knitr::kable(joint_tests(fit),caption="joint tests")
...
### Progressive, stable, contracting clones
...
p2.smoothed.vitc = ggplot(df.sp[!is.na(df.sp$value),],aes(x = 12*as.numeric(variable)/365.25,y=value2,color=Treatment_arm,fill=Treatment_arm))  +    geom_hline(aes(yintercept =50),color = colorRampPalette(c("steelblue3","black"))(5)[1],linewidth=1,linetype="dashed") +
...
##Similar to above but boxplots annoted with p-vals which are Tukey adjusted
...
table1(~mC.dG|Timepoint+Treatment_arm,data=df.merge)
...
table1(~hmC.mC|Timepoint+Treatment_arm,data=df.merge)
